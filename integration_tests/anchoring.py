...
def _canonical(param: dict) -> str:
    """A parameter's type as ``eth_abi`` takes it: a tuple spelled out from its components."""
    if param["type"].startswith("tuple"):
        return "(" + ",".join(_canonical(c) for c in param["components"]) + ")" + param["type"][5:]
    return param["type"]
...
LEAF_TYPES = [_canonical(a) for a in LEAF_APPENDED.abi.get("inputs", []) if not a.get("indexed")]
LEAVES_TYPES = [_canonical(a) for a in LEAVES_APPENDED.abi.get("inputs", []) if not a.get("indexed")]

#: One ``LeafAppended`` log, decoded. The event carries the peaks; ``root`` is what they bag to.
Leaf = namedtuple("Leaf", "namespace index commitment root peaks metadata")
#: One ``LeavesAppended`` log, decoded; ``chunks`` are ``(root, height)`` pairs, ``root`` the bag.
Leaves = namedtuple("Leaves", "namespace first count chunks root peaks metadata")
...
def decode_leaf(data) -> tuple[bytes, bytes, list[bytes], bytes]:
    ...
    """
    commitment, peaks, metadata = decode(LEAF_TYPES, bytes(HexBytes(data)))
    peaks = list(peaks)
    return commitment, bag(peaks), peaks, metadata
...
def batches_of(receipt) -> list[Leaves]:
    ...
    for lg in receipt["logs"]:
        if lg["address"].lower() != ANCHORING_ADDRESS.lower() or HexBytes(lg["topics"][0]) != LEAVES_APPENDED_TOPIC:
            continue
        count, chunks, peaks, metadata = decode(LEAVES_TYPES, bytes(HexBytes(lg["data"])))
        first = int.from_bytes(bytes(lg["topics"][2]), "big")
        pairs, peaks = [tuple(c) for c in chunks], list(peaks)
        out.append(Leaves(_namespace(lg["topics"][1]), first, count, pairs, bag(peaks), peaks, metadata))
    return out
