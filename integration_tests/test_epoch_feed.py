...
async def _traffic_until_fees(w3, faucet, recipients, tries=20):
    """Send cheap txs until one of `recipients` has collected fees; return it and the amount."""
...
async def _fee_stack_with_traffic(w3, faucet, *, commission_bps=1_000, stake=100 * ETHER):
    """A router per validator as its fee recipient, and traffic until one has earned.
    Returns (staking, routers, payouts, treasury, buybacks)."""
...
def election_net(request, tmp_path_factory):
    ...
    config = {
        ...
        "accounts": 200,
        # The election is read at each boundary and one test waits for three; 100 blocks would
        # be ~4 minutes of waiting. Long enough for the boundary DKG to finish.
...
class TestEpochFeed:
    ...
    async def test_committee_follows_staking_election(self, election_w3):
        ...
        d = await create(w3, LOCALNET_CHAIN_ID, deployer, deployer_initcode(PATH_USD))
    ...
    async def test_keeper_pays_out_real_chain_fees(self, election_w3):
        """The keeper pays out real block fees through the waterfall.

        Asserts conservation, not amounts: blocks keep landing while it runs. Runs last, since it
        repoints every validator's fee recipient.
        """
        w3 = election_w3
        faucet = Account.from_key(FAUCET_PRIVATE_KEY)
