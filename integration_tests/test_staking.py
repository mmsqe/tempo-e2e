...
class TestFeeRouting:
    ...
    async def test_flush_waterfalls_a_second_fee_token(self, w3, chain_id, funded_account):
        """A token other than the reward token still gets the cuts, and its delegator share is
        escrowed, even against a second flush. The misconfigured-or-donated case."""
...
class TestBridgeAndSwapper:
    ...
    async def test_guarded_swapper_caps_size_and_price_floor(self, w3, chain_id, funded_account):
        ...
        # Must revert on the cap itself, not PriceBelowFloor; `sender` passes the caller gate,
        # which would otherwise revert first and mask it.
