...
ZERO_CHUNK_ROOT = error_selector("ZeroChunkRoot()")
...
async def append_leaves(w3, chain_id, signer, chunks: list[tuple[bytes, int]], metadata=b""):
    return await send_call(w3, chain_id, signer, ADDR, A.fns.appendLeaves(chunks, metadata).data)
...
class TestAppend:
    async def test_append_moves_the_root_and_emits(self, w3, chain_id, anchorer):
    ...
    async def test_a_batch_from_empty_reaches_the_sequential_root(self, w3, chain_id, anchorer):
        ...
        assert batch.chunks == chunks and [h for _, h in chunks] == [3, 2, 0]
    ...
    async def test_a_chunk_off_the_alignment_is_refused(self, w3, chain_id, anchorer):
        ...
        data = A.fns.appendLeaves([chunk(6, 2)], b"").data
    ...
    async def test_an_empty_batch_is_a_noop_and_a_zero_chunk_root_is_refused(self, w3, chain_id, anchorer):
        """No chunks changes nothing and logs nothing; a zero root, which nothing hashes to,
        is refused."""
        await append(w3, chain_id, anchorer, c(1))

        receipt = await append_leaves(w3, chain_id, anchorer, [])
        assert batches_of(receipt) == [], "an empty batch logs nothing"

        data = A.fns.appendLeaves([(ZERO32, 0)], b"").data
        assert ZERO_CHUNK_ROOT in await call_revert(w3, ADDR, data, sender=anchorer.address)

        assert await root(w3, anchorer.address) == ROOTS[0]
...
class TestRefusedCalls:
    ...
    async def test_writes_are_rejected_in_a_read_only_frame(self, w3, chain_id, anchorer):
        ...
        # One aligned chunk, not an empty batch: that is a no-op, so it would prove nothing
        # about the frame.
...
class TestThroughAnIndexer:
    ...
    async def test_metadata_does_not_survive_tidx_decoding(self, w3, chain_id, anchorer, tidx):
        ...
        # The metadata's offset: past the three head words, the peaks' length and the peaks.
        offset = 3 * 32 + 32 + 32 * len(event.peaks)
