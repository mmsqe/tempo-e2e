...
Skipped unless ``ROOTED`` names the root, ``merkle``, ``sha256`` or ``mmr``. The plan is
written under ``REPLAY_DIR``; `corpus` says where the export and the migrator come from.
"""
...
async def test_rooted(w3, chain_id, tempo, factory):
    keys = await corpus.senders(w3, SENDERS)
    out = S / f"rooted-{ROOT}.jsonl"
    # Threshold zero: every registry goes in as a root, however few rows it holds.
    planned = corpus.plan(Path(corpus.EXPORT), out, "--threshold=0", f"--root={ROOT}")
