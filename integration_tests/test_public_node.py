...
def _wait_height(rpc_url: str, target: int, timeout: float = 90.0) -> int:
...
def test_public_node_syncs_blocks(two_network_net):
    ...
    later = _wait_height(public, start + 3)
...
def test_public_node_matches_follower_chain(two_network_net):
    ...
    target = max(1, _height(follower) - 2)
    assert _wait_height(public, target) >= target, "public node did not catch up to the follower"
